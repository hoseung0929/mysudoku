...
- 공용 퍼즐 카탈로그는 Firestore 또는 HTTP에서 내려받아 SQLite에 캐시합니다.
  - 관련 진입점: [`lib/database/database_manager.dart`](lib/database/database_manager.dart)
- 진행 중 세션은 `SharedPreferences`에 저장하고, 로그인 사용자가 있으면 Firestore에 동기화합니다.
...
- 공용 퍼즐 카탈로그: Firestore `puzzle_catalog`
- 오늘의 도전: Firestore `daily_challenges`
- 사용자 세이브: Firestore `users/{uid}/save_games`
- 관련 규칙 초안: [`firestore.rules`](firestore.rules)
- 관련 스키마 문서: [`FIRESTORE_SCHEMA.md`](FIRESTORE_SCHEMA.md)
- 실제 연결 가이드: [`FIREBASE_SETUP.md`](FIREBASE_SETUP.md)
- Firebase CLI 설정 템플릿: [`firebase.json`](firebase.json), [`.firebaserc.example`](.firebaserc.example)

현재 코드는 Firebase가 초기화되면 익명 로그인을 시도합니다. 다만 기기 간 이어하기를 실제로 보장하려면 Google/Apple/이메일 같은 영구 로그인 계정을 추가하는 것을 권장합니다.
...
5. `firebase deploy --only firestore:rules` 실행
6. `bash scripts/firestore_seed.sh export` 또는 `upload`로 Firestore 시드 반영
7. 필요 시 `bash scripts/firestore_daily_challenges.sh upload`로 오늘의 도전만 재생성
