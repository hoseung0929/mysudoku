...
**모든 UI 수정은 아이패드/아이폰을 분기해서 작업한다.** 아이패드 대응 변경이 아이폰 결과물에 영향을 주면 안 됨 — `isTablet` 플래그 분기 또는 폭 기반 clamp 상한 조정으로 처리하고, 수정 후 아이폰 시뮬레이터에서도 기존과 동일한지 확인할 것. 상세 원칙과 화면별 적용 현황은 [`docs/tablet-ui-guidelines.md`](docs/tablet-ui-guidelines.md) 참고.
